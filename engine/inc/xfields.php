...
// MWS Multi XFields
$lang_ex = array(
	'checkbox'		=> "Çek Kutusu",	// check box
	'radio'			=> "Radio Buton",	// radio buttun
);
// MWS Multi XFields
  ...
  case "configure":
    ...
    switch ($xfieldssubaction) {
      ...
      case "edit":
		...
		// MWS Multi XFields
		if ($editedxfield[3] == "select" || $editedxfield[3] == "radio" || $editedxfield[3] == "checkbox") {
			$options = array();
			$items = explode( "\r\n", $editedxfield["4_{$editedxfield[3]}"] );
		...
		// MWS Multi XFields
		unset($editedxfield["2_custom"], $editedxfield["4_text"], $editedxfield["4_textarea"], $editedxfield["4_select"], $editedxfield["4_radio"], $editedxfield["4_checkbox"]);
		// MWS Multi XFields
		...
		// MWS Multi XFields
		if ($editedxfield[3] == "text" OR $editedxfield[3] == "select" OR $editedxfield[3] == "radio" OR $editedxfield[3] == "checkbox") {
		// MWS Multi XFields
...
// MWS Multi XFields
		ShowOrHideEx("optional2", value == "text" || value == "select" || value == "radio" || value == "checkbox");
// MWS Multi XFields
...
// MWS Multi XFields
        ShowOrHideEx("optional", value != "select" || ( value == "radio" || value == "checkbox" ) );
        ShowOrHideEx("radio_options", value == "radio");
        ShowOrHideEx("checkbox_options", value == "checkbox");
// MWS Multi XFields
      }
...
?>
		<div class="form-group">
          ...
          <option value="select"<?php echo ($editedxfield[3] == "select") ? " selected" : "";?>><?php echo $lang['xfield_xsel']; ?></option>
			<!-- MWS Multi XFields -->
			<option value="radio"<?php echo ($editedxfield[3] == "radio") ? " selected" : "";?>><?php echo $lang_ex['radio']; ?></option>
			<option value="checkbox"<?php echo ($editedxfield[3] == "checkbox") ? " selected" : "";?>><?php echo $lang_ex['checkbox']; ?></option>
			<!-- MWS Multi XFields -->
...
<!-- MWS Multi XFields -->
		<div class="form-group" id="radio_options">
		  <label class="control-label col-lg-2"><?php echo $lang['xfield_xfaul']; ?></label>
		  <div class="col-lg-10">
			<textarea style="width: 320px; height: 100px;" name="editedxfield[4_radio]"><?php if ($editedxfield[4]{0} == "\r") $editedxfield[4] = "\n".$editedxfield[4]; echo ($editedxfield[3] == "radio") ? $editedxfield[4] : "";?></textarea><br><?php echo $lang['xfield_xfsel']; ?>
		  </div>
		 </div>
		<div class="form-group" id="checkbox_options">
		  <label class="control-label col-lg-2"><?php echo $lang['xfield_xfaul']; ?></label>
		  <div class="col-lg-10">
			<textarea style="width: 320px; height: 100px;" name="editedxfield[4_checkbox]"><?php if ($editedxfield[4]{0} == "\r") $editedxfield[4] = "\n".$editedxfield[4]; echo ($editedxfield[3] == "checkbox") ? $editedxfield[4] : "";?></textarea><br><?php echo $lang['xfield_xfsel']; ?>
		  </div>
		 </div>
<!-- MWS Multi XFields -->
...
// MWS Multi XFields
				elseif($value[3] == "radio") $type=$lang_ex['radio'];
				elseif($value[3] == "checkbox") $type=$lang_ex['checkbox'];
// MWS Multi XFields
...
// MWS Multi XFields
      } elseif ($value[3] != "select" OR $value[3] != "checkbox") {
// MWS Multi XFields
        $fieldvalue = $value[4];
      }
...
HTML;
			...
			foreach ( $items as $index => $value ) {
				$value = str_replace("'", "&#039;", $value);
				$output .= "<div class=\"multi-x-radio\"><input type=\"radio\" class=\"icheck\" name=\"xfield[{$fieldname}]\" value=\"{$value}\"" . ($fieldvalue == $value ? " checked" : "") . " /> <span>{$value}</span></div>\r\n";
			}
			$output .= <<<HTML
</div>
</div>
...
HTML;
			...
			foreach ( $items as $index => $value ) {
				$value = str_replace("'", "&#039;", $value);
				$output .= "<div class=\"multi-x-checkbox\"><input type=\"checkbox\" class=\"icheck\" name=\"xfield[{$fieldname}][]\" value=\"{$value}\"" . (in_array($value, $fieldvalue) ? " checked" : "") . " /> <span>{$value}</span></div>\r\n";
			}
			$output .= <<<HTML
</div>
</div>
...
// MWS Multi XFields
			if ( $key == "checkbox") {
				foreach ($val as $skey => $sval )	{
					$postedxfields[$key][$skey] = convert_unicode( $sval, $config['charset'] );
				}
			}
// MWS Multi XFields
...
// MWS Multi XFields
      if ($value[3] == "select" OR $value[3] == "radio" OR $value[3] == "checkbox") {
// MWS Multi XFields
        $options = explode("\r\n", $value[4]);
...
// MWS Multi XFields
		if (($value[8] == 1 OR $value[3] == "select" OR $value[3] == "radio" OR $value[3] == "checkbox") AND $xfield[$value[0]] != "" ) {
// MWS Multi XFields
